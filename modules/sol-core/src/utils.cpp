...
namespace sol
{
    std::vector<const char*> stringVectorToConstCharVector(const std::vector<std::string>& strings)
    {
        ...
        for (const auto& s : strings) cstrings.push_back(s.c_str());
        return cstrings;
    }

    uint32_t crc32(const uint8_t* data, const size_t size)
    {
        static constexpr uint32_t lookup[256] = {
        ...
        {
            const uint32_t lookupIndex = (checksum ^ data[i]) & 0xff;
            checksum                   = checksum >> 8 ^ lookup[lookupIndex];
        }
