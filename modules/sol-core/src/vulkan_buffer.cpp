...
namespace sol
{
    ...
    VulkanBuffer::VulkanBuffer(SettingsPtr         settingsPtr,
    ...
    VulkanBuffer::~VulkanBuffer() noexcept
    {
        if (settings->allocator)
            vmaDestroyBuffer(settings->allocator, buffer, allocation);
        else
            vkDestroyBuffer(settings->device, buffer, nullptr);
    }
    ...
    VulkanBufferPtr VulkanBuffer::create(Settings settings)
    {
        const auto [buffer, alloc, pMappedData] = createImpl(settings);
        return std::make_unique<VulkanBuffer>(std::make_unique<Settings>(settings), buffer, alloc, pMappedData);
    }

    VulkanBufferSharedPtr VulkanBuffer::createShared(Settings settings)
    {
        const auto [buffer, alloc, pMappedData] = createImpl(settings);
        return std::make_shared<VulkanBuffer>(std::make_unique<Settings>(settings), buffer, alloc, pMappedData);
    }
    ...
    std::tuple<VkBuffer, VmaAllocation, void*> VulkanBuffer::createImpl(const Settings& settings)
    {
        ...
        {
            handleVulkanError(vkCreateBuffer(settings.device, &bufferInfo, nullptr, &vkBuffer));
        }
    ...
    VulkanDevice& VulkanBuffer::getDevice() noexcept { return settings->device(); }

    const VulkanDevice& VulkanBuffer::getDevice() const noexcept { return settings->device(); }
    ...
    void VulkanBuffer::bindMemory(VulkanDeviceMemory& memory, const size_t offset)
    {
        ...
        if (settings->allocator) throw SolError("Cannot bind memory. Buffer was created using a custom allocator.");

        handleVulkanError(vkBindBufferMemory(settings->device, buffer, memory.get(), offset));
        deviceMemory = &memory;
    }
    ...
    {
        if (!deviceMemory && !allocation) throw SolError("Cannot set buffer data. No device memory bound.");
        if (size * count + offset > settings->size)
                                 ...
                                 const std::array<uint32_t, 2> regionSize) const
    {
        ...
        if (settings->size % (totalSize[0] * totalSize[1]) != 0)
        ...
        const auto elementSize = settings->size / (static_cast<size_t>(totalSize[0]) * totalSize[1]);
        ...
        {
            std::memcpy(static_cast<std::byte*>(mappedData) + regionOffset[1] * rowSize, data, settings->size);
            return;
        }
                                         ...
                                         const std::array<uint32_t, 2> regionSize) const
    {
        ...
        // Fill first row.
        const auto elementSize = settings->size / (static_cast<size_t>(totalSize[0]) * totalSize[1]);
        const auto offset      = (static_cast<size_t>(regionOffset[0]) + regionOffset[1] * totalSize[0]) * elementSize;
    ...
    {
        if (allocation)
            handleVulkanError(vmaMapMemory(settings->allocator, allocation, &mappedData));
        else
            handleVulkanError(vkMapMemory(settings->device, deviceMemory->get(), 0, settings->size, 0, &mappedData));
    }
    ...
    void VulkanBuffer::unmap()
    {
        ...
        if (allocation)
            vmaUnmapMemory(settings->allocator, allocation);
        else
            vkUnmapMemory(settings->device, deviceMemory->get());
    }

    void VulkanBuffer::flush() const
    {
        if (allocation)
            vmaFlushAllocation(settings->allocator, allocation, 0, VK_WHOLE_SIZE);
        else
            throw SolError("");
    }
}  // namespace sol
